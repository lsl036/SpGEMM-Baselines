...
BIN_PATH="${CLUSTERWISE_ROOT}/bin/ShuffleRowIDs_hw"

# 在脚本中添加
SHUFFLED_DATA_PATH="/data2/linshengle_data/SpGEMM-Reordering/shuffle_order"
...
save_on_datasets() {
  ...
  for dataset in "${datasets[@]}"; do
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~<DATASET: ${dataset}>~~~~~~~~~~~~~~~~~~~~~~~~~"
    $BIN_PATH text \
      "${DATA_PATH}/${dataset}/${dataset}.mtx" -s -o "${SHUFFLED_DATA_PATH}"
    echo ""
  done
}

# Execute the benchmark:
save_on_datasets "${initial_datasets[@]}"
save_on_datasets "${ispass_datasets[@]}"
save_on_datasets "${new_datasets[@]}"
