...
mypaper_dataset=(
  "cant" "pwtk" "offshore" "cage12" "scircuit" "wiki-Vote" "poisson3Da" 
...
BIN_PATH="${CLUSTERWISE_ROOT}/bin/ReorderedVlengthClusterSpGEMM_hw"

# Function that takes dataset names as arguments
run_on_datasets() {
  local datasets=("$@")
  for dataset in "${datasets[@]}"; do
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~<DATASET: ${dataset}-GPOrder>~~~~~~~~~~~~~~~~~~~~~~~~~"
    $BIN_PATH text \
      "${DATA_PATH}/${dataset}/${dataset}.mtx" \
      "${GP_DATA_PATH}/${dataset}.gporder"
    echo ""
  done
}
