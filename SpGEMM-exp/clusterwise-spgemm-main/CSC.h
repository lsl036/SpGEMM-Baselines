...
size_t CSC<IT,NT>::calculate_size() {
  size_t total = sizeof(CSC<IT, NT>);  // size of the class instance itself
  total += ((cols + 1) * sizeof(IT));             // colptr
  total += (nnz * sizeof(IT));                    // rowids
  total += (nnz * sizeof(NT));                    // values
  // Convert bytes to gigabytes
  double size_in_gb = static_cast<double>(total) / (1024 * 1024 * 1024);
  cout << "Size of CSC matrix: " << size_in_gb << " GB" << endl;
  return total;
}
    ...
    if (G.start[k] != G.start[k + 1]) {
      ...
      for (int i = 0; i < currCol.size() - 1; ++i) {
        if (currCol[i].first == currCol[i + 1].first) {
          ...
          simpleG.push_back(
              Triple<IT, NT>(currCol[i + 1].first.first, currCol[i + 1].first.second, currCol[i + 1].second));
        }
      }
...
#pragma omp parallel for
  ...
  for (IT j = 0; j < nnz; ++j) {
    ...
    std::cout << " read_id : " << rowids[j] << " kmer_id : " << get<1>(tuple[j]) << " pos_in_read : " << values[j]
              << endl;
...
bool CSC<IT, NT>::operator==(const CSC<IT, NT> &rhs) {
  ...
  if (samebefore && (!same)) {
    ...
    if (error_original_pair.size() > 10)  // otherwise would crush for small data
    {
      ...
      for (typename vector<pair < NT, NT> > ::iterator it = error_original_pair.begin(); it !=
                                                                                         error_original_pair.end();
      ++it)
