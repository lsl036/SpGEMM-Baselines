...
{
    printf("METIS Matrix Partitioning Tool\n");
    printf("==============================\n\n");
    printf("Usage: %s -i <matrix-file> -k <num-partitions> -o <objective> [OPTIONS]\n\n", program_name);
    printf("Required Arguments:\n");
    printf("  -i <matrix-file>       Path to the Matrix Market format file (.mtx)\n");
    printf("  -k <num-partitions>    Number of partitions (k-way partitioning, e.g., 64)\n");
    printf("  -o <objective>         Partitioning objective:\n");
    printf("                           - edge-cut  : Minimize edge-cut (edge cut minimization)\n");
    printf("                           - volume    : Minimize total communication volume\n\n");
    printf("Optional Arguments:\n");
    printf("  -t, --time             Print elapsed time information to stdout\n");
    printf("  -s, --save             Save partition results to files\n");
    printf("  -h, --help             Show this help message\n\n");
    printf("Output Files (when -s is used):\n");
    printf("  The program generates the following files in the 'output/' and 'status/' directories:\n");
    printf("    - output/<matrix>_metis_<objective>_part<num>.txt          : Partition vector\n");
    printf("    - output/<matrix>_metis_<objective>_part<num>_info.txt      : Partition information\n");
    printf("    - status/<matrix>_metis_<objective>_part<num>_status.txt   : Timing information\n\n");
    printf("Examples:\n");
    printf("  %s -i matrix.mtx -k 64 -o edge-cut -t -s\n", program_name);
    printf("  %s -i matrix.mtx -k 32 -o volume -s\n", program_name);
    printf("  %s -i matrix.mtx -k 16 -o edge-cut -t\n\n", program_name);
}
...
int main(int argc, char *argv[]){
    ...
    for(int j = 0; j < arguments.size(); j++){
        if(arguments[j] == "-h" || arguments[j] == "--help"){
            PrintHelp(argv[0]);
            exit(0);
        }
    }
    ...
    // Parse arguments
        for(int j = 0; j < arguments.size(); j++){
            ...
            if(j + 1 >= arguments.size()){
                fprintf(stderr, "Error: -i option requires a file path.\n\n");
                PrintHelp(argv[0]);
                exit(1);
            }
            ...
            if(arguments[j] == "-k"){
            if(j + 1 >= arguments.size()){
                fprintf(stderr, "Error: -k option requires a partition number.\n\n");
                PrintHelp(argv[0]);
                exit(1);
            }
            try {
            ...
            if(arguments[j] == "-o"){
            if(j + 1 >= arguments.size()){
                fprintf(stderr, "Error: -o option requires an objective type.\n\n");
                PrintHelp(argv[0]);
                exit(1);
            }
                ...
                fprintf(stderr, "Error: Invalid objective type: %s\n", arguments[j+1].c_str());
                fprintf(stderr, "Valid options are: 'edge-cut' or 'volume'\n\n");
                PrintHelp(argv[0]);
                exit(1);
                }
            }
    ...
    if(filePath.empty()){
        fprintf(stderr, "Error: Missing required argument -i (input file).\n\n");
        PrintHelp(argv[0]);
        exit(1);
    }
    
    // Check if -k was provided
    bool kProvided = false;
    for(int j = 0; j < arguments.size(); j++){
        if(arguments[j] == "-k"){
            kProvided = true;
            break;
    ...
    if(!kProvided){
        fprintf(stderr, "Error: Missing required argument -k (number of partitions).\n\n");
        PrintHelp(argv[0]);
        exit(1);
    }
    
    if(objective_name.empty()){
        fprintf(stderr, "Error: Missing required argument -o (objective type).\n\n");
        PrintHelp(argv[0]);
        exit(1);
    }
    ...
    if((f = fopen(filePath.c_str(),"r")) == NULL){
        ...
        fprintf(stderr, "Please make sure the file path is correct and the file exists.\n\n");
        PrintHelp(argv[0]);
        exit(1);
    }
