...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    cout << "Usage: ./gray_order <matrix_market_file> <reorder_output_file> <save_to_file>\n";
    cout
  ...
  if (save_to_file) {
    ...
    std::copy(&gray_reorder[0], &gray_reorder[num_rows],
              std::ostream_iterator<vertex_type>(gray_order_file, "\n"));
