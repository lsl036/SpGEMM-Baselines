...
# Run ND reordering following the requested pattern:
# echo "~~~~~~~~~~~~~~~~~~~~~~~~~<DATASET: <name>-ND>~~~~~~~~~~~~~~~~~~~~~~~~~"
# mtxreorder --verbose <DATA_PATH>/<name>/<name>.mtx --ordering=nd \
#   --rowperm-path=<DATA_PATH>/reordering/nd_order/<name>.ndorder \
#   > <DATA_PATH>/reordering/nd_order/<name>_nd.mtx
run_nd_on_datasets() {
  local data_path="$1"; shift
  local datasets=("$@")

  for dataset in "${datasets[@]}"; do
    echo "~~~~~~~~~~~~~~~~~~~~~~~~~<DATASET: ${dataset}-ND>~~~~~~~~~~~~~~~~~~~~~~~~~"
    ${BIN} "${data_path}/${dataset}/${dataset}.mtx" \
      --ordering=nd \
      --rowperm-path="${ND_DATA_PATH}/${dataset}.ndorder" \
      > "${ND_DATA_PATH}/${dataset}_nd.mtx"
    echo ""
  done
}

echo "ND reordering ..."
run_nd_on_datasets "${DATA_PATH}" "${initial_datasets[@]}"
run_nd_on_datasets "${DATA_PATH}" "${ispass_datasets[@]}"
run_nd_on_datasets "${DATA_PATH}" "${new_datasets[@]}"
