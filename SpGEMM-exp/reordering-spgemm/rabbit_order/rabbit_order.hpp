...
std::unique_ptr<vint[]> compute_perm(const graph& g,
  ...
  assert(offsets.size() == ncom + 1);

//#ifdef SAVE_TO_FILE
  if(save_to_file) {
    std::ofstream reorder_file(reorder_path.c_str());
    std::ofstream commu_file(commu_path.c_str());

    assert(reorder_file.is_open() && "Reorder output file is not open!");
    assert(commu_file.is_open() && "Community output file is not open!");

    std::copy(&perm[0], &perm[g.n()], std::ostream_iterator<vint>(reorder_file, "\n"));
//  std::copy(&coms[0], &coms[g.n()], std::ostream_iterator<vint>(commu_file, "\n"));
    std::copy(&offsets[0], &offsets[ncom + 1], std::ostream_iterator<vint>(commu_file, "\n"));
